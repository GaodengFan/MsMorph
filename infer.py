...
def main():
    ...
    model_idx = -1
    channels = 16
    model_folder = 'MSMorph_ncc_{}_reg_{}_lr_{}_channels_{}/'.format(*weights, lr,channels)
    model_dir = 'experiments/' + model_folder
    if not os.path.exists('result' ):
        os.makedirs('result')
    f = open(os.path.join('result' , 'MSMorph' + ".txt"), "a")
    ...
    test_set = datasets.LPBABrainInferDatasets(glob.glob(val_dir + '*.pkl'), transforms=test_composed)
