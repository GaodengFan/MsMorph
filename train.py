...
def main():
    batch_size = 1
    train_dir = 'your data_pth'
    val_dir = 'your data_pth'

    weights = [1, 1]  # loss weights
    lr = 0.0001
    channels = 16
    save_dir = 'MSMorph_ncc_{}_reg_{}_lr_{}_channels_{}/'.format(*weights, lr,channels)
    ...
    Initialize model
    '''
    model = MSMorph(img_size, channels=channels)
    model.cuda()
    ...
    train_set = datasets.LPBABrainTrainDatasets(glob.glob(train_dir + '*.pkl'), transforms=train_composed)
    val_set = datasets.LPBABrainInferDatasets(glob.glob(val_dir + '*.pkl'), transforms=val_composed)
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        writer.add_scalar('Loss', loss_all.avg, epoch)
        '''
        Validation
        '''
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                print(epoch, ':', eval_dsc.avg,'+-',eval_dsc.std)
        best_dsc = max(eval_dsc.avg, best_dsc)
        print(eval_dsc.avg,'+-',eval_dsc.std, file=f)
        # writer.add_scalar('loss', eval_dsc.avg, epoch)
        writer.add_scalar('Eval_DSC', eval_dsc.avg, epoch)
        save_checkpoint({
            'epoch': epoch + 1,
